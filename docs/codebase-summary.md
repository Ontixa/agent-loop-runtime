...
| `src/engine/` | `mission-factory`, `mission-runner`, `planner`, `task-graph`, `validation-gates`, `reviewer`, `scheduler`, `admission-control`, `recovery`, `scope-expansion`, `mission-presets` (+ `builtin-presets`) — strict-scope maintenance templates resolving to spec + tightened policy |
| `src/config/config-manager.ts` | `agentloop.config.json` load/validate + `qwen-loop.config.json` migration |
| `src/daemon/` | `daemon` (lifecycle, stale sweep, signal handling), `control-api` (loopback HTTP v1 — frozen contract in docs/control-api-contract.md), `socket-transport` (Unix socket / Windows named-pipe bind: validation, stale-endpoint reclaim, release), `event-follow` (`?follow` NDJSON tail) |
